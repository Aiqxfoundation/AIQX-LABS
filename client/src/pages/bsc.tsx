...
export default function BSC() {
  ...
  const deployMutation = useMutation({
    mutationFn: async (data: EvmTokenCreationForm) => {
      ...
      try {
        const response = await apiRequest("POST", "/api/tokens/deploy", {
          ...data,
          chainId: data.chainId || 'bsc-testnet',
          blockchainType: "EVM",
    ...
    onSuccess: (data) => {
      toast({
        title: "BSC Token Deployed Successfully!",
  ...
  return (
    <div className="container max-w-5xl py-12 px-4">
      <div className="mb-8 flex items-center gap-4">
        <div className="h-16 w-16 rounded-full bg-gradient-to-br from-yellow-500 to-yellow-600 p-0.5">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Deploy your custom BEP20 token on Binance Smart Chain Mainnet or Testnet
          </p>
        </div>
      </div>

      {!isConnected && (
        <Card className="mb-6 border-2 border-yellow-500/20 bg-gradient-to-br from-yellow-500/5 to-yellow-600/5">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                data-testid="button-connect-bsc-wallet"
              >
      ...
      <TokenCreationFormComponent
        ...
        defaultChainId="bsc-testnet"
        allowedChainIds={['bsc-mainnet', 'bsc-testnet']}
      />
    </div>
  );
}
