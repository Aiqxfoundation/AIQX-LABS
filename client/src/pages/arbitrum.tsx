...
export default function Arbitrum() {
  ...
  const deployMutation = useMutation({
    mutationFn: async (data: EvmTokenCreationForm) => {
      ...
      try {
        const response = await apiRequest("POST", "/api/tokens/deploy", {
          ...data,
          chainId: data.chainId || 'arbitrum-testnet',
          blockchainType: "EVM",
    ...
    onSuccess: (data) => {
      toast({
        title: "Arbitrum Token Deployed Successfully!",
  ...
  return (
    <div className="container max-w-5xl py-12 px-4">
      <div className="mb-8 flex items-center gap-4">
        <div className="h-16 w-16 rounded-full bg-gradient-to-br from-cyan-500 to-cyan-600 p-0.5">
        ...
        <div>
          <h1 className="text-4xl font-bold bg-gradient-to-r from-cyan-500 to-cyan-600 bg-clip-text text-transparent">
            Create Arbitrum Token
          </h1>
          <p className="text-muted-foreground">
            Deploy your custom token on Arbitrum One or Sepolia Testnet
          </p>
        </div>
      </div>

      {!isConnected && (
        <Card className="mb-6 border-2 border-cyan-500/20 bg-gradient-to-br from-cyan-500/5 to-cyan-600/5">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                data-testid="button-connect-arbitrum-wallet"
              >
      ...
      <TokenCreationFormComponent
        ...
        defaultChainId="arbitrum-testnet"
        allowedChainIds={['arbitrum-mainnet', 'arbitrum-testnet']}
      />
    </div>
  );
}
